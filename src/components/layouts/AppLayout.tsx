...
const AppLayout: React.FC = () => {
  const { view, error, resetApp, currentStepIndex, analysisResult } = useAnalysis();
    ...
    <div className="app-container">
          ...
          <Suspense fallback={<div>Loading content...</div>}>
            ...
            {view === 'loading' && (
              <div className="max-w-4xl mx-auto mt-12">
                <ThinkingProcess steps={analysisSteps} currentStep={currentStepIndex} />
              </div>
            )}
            ...
            {view === 'error' && (
              <div className="max-w-4xl mx-auto">
                <div className="card mt-6 p-4 flex-col items-center gap-4 text-center" style={{ borderColor: 'var(--color-red)' }}>
                  ...
                  <button onClick={resetApp} className="button button-secondary mt-4">Start Over</button>
                </div>
              </div>
            )}
      ...
      <footer className="text-center p-6 opacity-50 text-sm" role="contentinfo">
        ...
        <button
          ...
          className="chromatic-aberration mt-4 text-sky"
          style={{ background: 'none', border: 'none', cursor: 'pointer', fontSize: '0.75rem', textTransform: 'uppercase' }}
