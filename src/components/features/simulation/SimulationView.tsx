            ...
            {simulationResult.timelessJewel && (
                <AnalysisCard title="Timeless Jewel" icon="gem">
                    ...
                    <ul className="flex-col gap-2 text-sky" style={{listStyle: 'none'}}>
                        {simulationResult.timelessJewel.notableStats.map((stat: string, i: number) => <li key={i} className="flex-row items-center gap-2"><Icon name="star" style={{width:'1rem', height:'1rem'}} />{stat}</li>)}
                    </ul>
                </AnalysisCard>
            )}
                    ...
                    <div className="flex-col gap-4">
                        {simulationResult.clusterJewels.map((jewel: any, i: number) => (
                             <div key={i} className="card p-4" style={{backgroundColor: 'rgba(0,0,0,0.2)'}}>
                                ...
                                <ul className="flex-col gap-2 text-sky" style={{listStyle: 'none'}}>
                                   {jewel.notablePassives.map((passive: string, j: number) => <li key={j} className="flex-row items-center gap-2"><Icon name="star" style={{width:'1rem', height:'1rem'}} />{passive}</li>)}
                                </ul>
                            </div>
                        ))}
                    </div>
                            ...
                            <div key={i} className="card p-4" style={{backgroundColor: 'rgba(0,0,0,0.2)'}}>
                                ...
                                <div className="grid gap-4" style={{gridTemplateColumns: '1fr 1fr'}}>
                                    <div>
                                        ...
                                        <ul className="text-sm flex-col gap-1" style={{listStyle: 'none'}}>{opt.pros.map((pro: string, j: number) => <li key={j}>+ {pro}</li>)}</ul>
                                    </div>
                                    <div>
                                        <h5 className="font-bold text-red mb-2">CONS</h5>
                                        <ul className="text-sm flex-col gap-1" style={{listStyle: 'none'}}>{opt.cons.map((con: string, j: number) => <li key={j}>- {con}</li>)}</ul>
                                    </div>
                                </div>
                            </div>
                       ))}
