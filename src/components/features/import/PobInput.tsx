...
const ManualImportView: React.FC = () => {
    ...
    useEffect(() => {
        const handler = setTimeout(() => {
            ...
            setValidationStatus({ status: 'validating', message: '' }); // Message will be shown via showError
            if (isPobCode(input)) {
                try {
...
export const PobInput: React.FC = () => {
    ...
    <div className="card p-6 flex-col gap-6">
        ...
        <div id="import-panel" role="tabpanel" aria-labelledby={`tab-${importMethod}`} className="pt-4">
            {showPreflight ? (
                ...
                importMethod === 'account' ? <AccountImportView /> : <ManualImportView />
            )}
        </div>
